...
class Simplex:
    ...
    def init(self, tableau, numSlackVar):
        ...
        self.matrix.aux = True
        self.adjustMatrix()

        # self.printa_cOiSaS(self.matrix.tableau_aux)
        # print('pivoteia para ajusta aux')
        # # restaura bases encontradas com a PL auxiliar
        # for i in range(0, len(self.matrix.basis)):
        #     self.matrix.pivoting(self.matrix.basis[i][0], self.matrix.basis[i][1])

        print(self.matrix.basis)
        self.printa_cOiSaS(self.matrix.tableau_aux)
    ...
    def adjustMatrix(self):
        ...
        for i in range(1, self.matrix.getB().shape[0]):
            if (self.matrix.getB()[i, 0] < 0):
                self.matrix.tableau_aux[i, :] = -1 * self.matrix.tableau_aux[i, :]
    ...
    def testNonViable(self):
        for i in range(1, self.matrix.getB().shape[0]):
            # flag de validade: começa considerando a linha correspondente ao b negativo como inválida. Se achar pelo
            # menos um elemento negativo na matriz A, a linha passa a ser válida
            valid = False
            # procura b negativo
            if self.matrix.getB()[i, 0] < 0:
                ...
                for j in range(0, self.matrix.getA().shape[1]):
                    ...
                    if self.matrix.getA()[i - 1, j] < 0:
                        valid = True
                        break
                # se passou pela linha do A inteira e não achou ninguém negativo, a PL é inviável e essa linha
                #  é problemática
                if valid is False:
                    return index, False
        # caso não tenha entrado no critério para nenhuma das linhas, retorna uma linha não existente e confirma
        #  viabilidade da PL
        return -1, True
    ...
    def testUnbounded(self):
        for i in range(0, self.matrix.getC().shape[1]):
            # flag de validade: começa considerando a coluna correspondente ao -c negativo como inválida.
            # Se achar pelo menos um elemento positivo na matriz A, a linha passa a ser válida
            valid = False
            # procura -c negativo
            if self.matrix.getC()[0, i] < 0:
                ...
                for j in range(0, self.matrix.getA().shape[0]):
                    ...
                    if self.matrix.getA()[j, i] > 0:
                        valid = True
                        break
                # se passou pela coluna do A inteira e não achou ninguém positivo, a PL é ilimitada e essa
                # coluna é problemática
                if valid is False:
                    return index, False
        # caso não tenha entrado no critério, retorna uma coluna não existente e confirma limitabilidade da PL
        return -1, True

    # se confirmado caso de invialibilidade (tanto na PL auxiliar como no dual simplex), esta função que trata o caso
    # e extrai o certificado ----------------------------------------------------------------------------------------
    ...
    def unbounded(self, cIndex=None):
        ...
        for i in range(0, self.matrix.getA().shape[1]):
            for j in self.matrix.basis:
                # para cada coluna de A, confere se ela é da base de soluções
                if (self.matrix.getOpMatrix().shape[1] + i) == j[1]:
    ...
    def primalPivot(self):
        ...
        return self.matrix.getC().shape[0] + rIndex, self.matrix.getOpMatrix().shape[1] + cIndex
    ...
    def runPrimal(self):
        ...
        # self.matrix.printa_cOiSaS(self.matrix.tableau_aux)
        self.selection()
    ...
    def dualPivot(self):
        ...
        for i in range(0, self.matrix.getA().shape[1]):
            if self.matrix.getA()[rIndex, i] < 0:
                # calcula a razão (em valor absoluto) entre o A negativo na linha do b escolhido e o -c
                # da coluna correspondente ao A
    ...
    def getSolution(self):
        ...
        for i in range(0, (self.matrix.getA().shape[1] - self.matrix.getSlack().shape[1])):
            for j in self.matrix.basis:
        ...
        for i in range(numRows):
            ...
            for j in range(numColumns+1):
                if (FPIMatrix[i,j].denominator == 1):
                    print("%3d" % FPIMatrix[i,j].numerator, sep="", end="")
                else:
                    print("%3d" % FPIMatrix[i,j].numerator, '/', FPIMatrix[i,j].denominator, sep="", end="")
            if i == 0: print(" | <- (C|VO)")
            else: print(" | <- (A|b)")
        # print("==========================================")